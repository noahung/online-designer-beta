// Fixed Comprehensive Diagnostic Test for Webhook and Email Systems
...
async function comprehensiveDiagnostic() {
  ...
  let supabase = null
  try {
    // Try to get Supabase from window object (if exposed globally)
    if (window.supabase) {
      supabase = window.supabase
      console.log('✅ Found Supabase client in window object')
    } else {
      // Try to access it through the React app
      const appRoot = document.getElementById('root')
      if (appRoot && appRoot._reactInternalInstance) {
        // This is a more complex approach - let's try a simpler method
        console.log('⚠️ Supabase client not found in window. Please ensure you\'re logged into the application.')
        console.log('💡 Try running this diagnostic from the main dashboard page after logging in.')
        return
      }
    }
  } catch (e) {
    console.log('⚠️ Could not access Supabase client automatically')
    console.log('💡 Please run this diagnostic from the main application page after logging in')
    return
  }

  if (!supabase) {
    console.log('❌ Supabase client not available. Please run this from the main application page.')
    return
  }
  ...
  try {
    ...
    results.emailSystem.sendTest = emailTestResult
    ...
    console.log(`  • Sending function: ${results.emailSystem.sendTest.status} (${results.emailSystem.sendTest.endpoint})`)
